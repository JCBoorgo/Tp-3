...
public class ControleurMemoire implements ActionListener, Runnable {
	...
	public static void bienvenu() {
		...
		String sMessage = "Éducation physique pour votre mémoire :o)"
				...
				+ "et de les sélectionnées dans le même ordre après.\n\n"
				+ "Si vous réussissez vous augmenterez de niveau et vous aurez plus de formes à mémoriser.\n\n"
				+ "Bonne chance et amusez-vous bien!...";

		JOptionPane.showMessageDialog(null, sMessage, sTitre, JOptionPane.INFORMATION_MESSAGE);
	}
	...
	private void ajouterEcouteur() {
		...
		for (int i = 0; i < JeuMemoire.LIGNE; i++) {
			for (int j = 0; j < JeuMemoire.COLONNE; j++) {
				...
				vue.boutons[i][j].setIcon(new ImageIcon("images\\" + image + ".png"));
			}
		}
	}
	...
	private void verifierBouton(JButton b) {
		...
		if (fin == 1 || fin == 2) {
			...
			fini = true;
			if (fin == 1)
				JOptionPane.showMessageDialog(null, "Erreur, on recommence ce niveau");
			else {
				if (jeu.getNiveau() < Memorable.NIVEAU_MAX) {
					JOptionPane.showMessageDialog(null, "On change de niveau");
					jeu.setNiveauPlusUn();
				} else {
					JOptionPane.showMessageDialog(null, "Niveau maximum atteint, on reste à ce niveau");
				}
			}
			mettreAJourNiveau();
			t1 = new Thread(this);
